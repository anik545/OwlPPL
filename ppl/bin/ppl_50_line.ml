...
let multinomial_sample (ps: float list) :int=
  ...
  let rec loop ps_left remaining curr_idx = 
    match ps_left with 
      x::xs -> if (Float.(<) x remaining) then loop xs (remaining -. x) (curr_idx + 1) else curr_idx
    | [] -> curr_idx
  in 
  loop ps u 0
...
let resample ?(n=300) (prob: 'a prob) : 'a prob =
  ...
  let mx = match List.reduce lw ~f:Float.max with
    | Some x -> x
    | None -> 0. in
...
let unweighted ?(lw=0.) (ts: 'a list) : 'a prob = List.map ts ~f:(fun t -> (t,lw))

(* All these methods for a uniform on [a,b] *)

type uniform = float*float

let ll ((a, b): uniform)  (obs:float) = if Float.(<) obs a || Float.(>) obs b then 0.0 else log (1. /. (b -. a))

let ll_list ((a,b):uniform) (obss) = floatSum (List.map obss ~f:(ll (a,b)))

let fit ((a,b):uniform) (particles: 'a prob) (obs: 'a list) : 'a prob  = List.map particles ~f:(fun (v,lw) -> (v, lw +. ll_list (a,b) (obs) ))

let fitQ ((a,b):uniform) (_: 'a prob) (obs: 'a list): 'a prob = match obs with 
    x::xs -> [(x, ll_list (a,b) xs)]
  | [] -> []

let particles_uniform ?(n=300) (a,b)  = unweighted (List.init n ~f:(fun _ -> uniform_rvs ~a:a ~b:b))

let empirical (particles: 'a prob): 'a list = List.map ~f:(fun (v,_) -> v) (resample particles)
...
let mean_var_a = (List.map ~f:(fun (aV,_) -> aV) emp) |> Array.of_list |> mean_var
let mean_var_b = (List.map ~f:(fun (_,bV) -> bV) emp) |> Array.of_list |> mean_var
