...
type _ dist_type = 
    Continuous: float dist -> float dist_type
  | Discrete: 'a dist -> 'a dist_type
...
let print_kls_continuous model ?(n=1000) infer_strats true_dist = 
  ...
  let kls = List.map inferred_dists ~f:(fun (x,d) -> x,KL.kl_continuous ~n true_dist d) in
  List.iter kls ~f:(fun (s,kl) -> Printf.printf "%s - kl= %f\n" (show_infer_strat s) kl);
  Printf.printf "\n"

let print_kls_discrete ?(n=1000) model infer_strats true_dist = 
  let inferred_dists = List.map infer_strats ~f:(fun x -> (x,infer model x)) in
  let kls = List.map inferred_dists ~f:(fun (x,d) -> x,KL.kl_discrete ~n true_dist d) in
  List.iter kls ~f:(fun (s,kl) -> Printf.printf "%s - kl=%f\n" (show_infer_strat s) kl);
  Printf.printf "\n"
...
let sprinkler () = 
  let infer_strats = [Enum; Rejection(100, Hard); Rejection(100,Soft); MH(10); MH(100); MH(1000)] in
  let ri = Primitives.categorical [(true,0.707927677329624472); (false,0.292072322670375473)] in
